...
namespace Global.System.Updaters.Logs
{
    public class UpdaterLogger
    {
        ...
        public void OnGizmosUpdatableAdded(int count)
        {
            ...
            _logger.Log($"Gizmos updatable added, count: {count}", _settings.LogParameters);
        }

        public void OnGizmosUpdatableRemoved(int count)
        {
            if (_settings.IsAvailable(UpdaterLogType.GizmosUpdatableRemove) == false)
                return;

            _logger.Log($"Gizmos updatable removed, count: {count}", _settings.LogParameters);
        }

        public void OnGizmosUpdateCalled(int count)
        {
            if (_settings.IsAvailable(UpdaterLogType.GizmosUpdatableCalled) == false)
                return;

            _logger.Log($"Gizmos update called for {count} listeners", _settings.LogParameters);
        }
