...
namespace GamePlay.Player.Entity.Views.Physics.Runtime
{
    ...
    public class PlayerPhysicsView : MonoBehaviour
    {
        ...
        private readonly List<PhysicsVolume> _physicsVolumes = new();
        ...
        /// </summary>
        private void OnCustomMovementMode(float deltaTime)
        {
            // Trigger event
        ...
        /// </summary>
        private void OnCustomRotationMode(float deltaTime)
        {
        ...
        /// This 'hook' lets you externally update the character 'state'.
        /// </summary>
        private void OnBeforeSimulationUpdate(float deltaTime)
        {
        ...
        /// This 'hook' lets you externally update the character 'state'. 
        /// </summary>
        private void OnAfterSimulationUpdate(float deltaTime)
        {
        ...
        /// </summary>
        private void SetPhysicsVolume(PhysicsVolume newPhysicsVolume)
        {
        ...
        /// </summary>
        private void OnPhysicsVolumeChanged(PhysicsVolume newPhysicsVolume)
        {
        ...
        private void UpdatePhysicsVolume(PhysicsVolume newPhysicsVolume)
        {
            ...
            var characterCenter = characterMovement.worldCenter;
        ...
        /// </summary>
        private void AddPhysicsVolume(Collider other)
        {
        ...
        /// </summary>
        private void RemovePhysicsVolume(Collider other)
        {
            if (other.TryGetComponent(out PhysicsVolume volume) && _physicsVolumes.Contains(volume))
        ...
        /// </summary>
        private void UpdatePhysicsVolumes()
        {
        ...
        /// </summary>
        private bool IsInWaterPhysicsVolume()
        {
        ...
        /// </summary>
        private void RotateTowards(Vector3 worldDirection, float deltaTime, bool updateYawOnly = true)
        {
            var characterUp = GetUpVector();
            ...
            var targetRotation = Quaternion.LookRotation(worldDirection, characterUp);
        ...
        /// </summary>
        private void AddPitchInput(float value)
        {
        ...
        /// </summary>
        private void AddRollInput(float value)
        {
        ...
        /// </summary>
        private void ConsumeRotationInput()
        {
        ...
        public void SetMovementMode(MovementMode newMovementMode, int newCustomMode = 0)
        {
            ...
            var prevMovementMode = _movementMode;
            var prevCustomMode = _customMovementMode;
        ...
        /// The ground speed that we should accelerate up to when walking at minimum analog stick tilt.
        /// </summary>
        private float GetMinAnalogSpeed()
        {
            switch (_movementMode)
            {
                case MovementMode.Walking:
        ...
        /// </summary>
        private float GetMaxBrakingDeceleration()
        {
            switch (_movementMode)
            {
                case MovementMode.Walking:
        ...
        /// </summary>
        private float ComputeAnalogInputModifier(Vector3 desiredVelocity)
        {
        ...
        /// </summary>
        private Vector3 ApplyVelocityBraking(Vector3 velocity, float friction, float maxBrakingDeceleration,
            float deltaTime)
        {
            ...
            var isZeroFriction = friction == 0.0f;
            var isZeroBraking = maxBrakingDeceleration == 0.0f;
            ...
            var oldVel = velocity;
            var revAccel = isZeroBraking ? Vector3.zero : -maxBrakingDeceleration * velocity.normalized;
            ...
            while (remainingTime >= kMinTickTime)
            {
                ...
                var dt = remainingTime > kMaxTimeStep && !isZeroFriction
            ...
            var sqrSpeed = velocity.sqrMagnitude;
        ...
        /// </summary>
        private Vector3 CalcVelocity(Vector3 velocity, Vector3 desiredVelocity, float friction, bool isFluid,
            float deltaTime)
        {
            const float kMinTickTime = 0.000001f;
            ...
            var desiredMoveDirection = desiredSpeed > 0.0f ? desiredVelocity / desiredSpeed : Vector3.zero;

            // Requested acceleration (factoring analog input)

            var analogInputModifier = ComputeAnalogInputModifier(desiredVelocity);
            var inputAcceleration = GetMaxAcceleration() * analogInputModifier * desiredMoveDirection;

            // Actual max speed (factoring analog input)

            var actualMaxSpeed = Mathf.Max(GetMinAnalogSpeed(), GetMaxSpeed() * analogInputModifier);

            // Apply braking or deceleration

            var isZeroAcceleration = inputAcceleration.isZero();
            var isVelocityOverMax = velocity.isExceeding(actualMaxSpeed);
            ...
            if (isZeroAcceleration || isVelocityOverMax)
            {
                ...
                var actualBrakingFriction = useSeparateBrakingFriction ? brakingFriction : friction;
                var actualBrakingAcceleration =
            ...
            else
            {
                ...
                var accelDir = inputAcceleration.normalized;
            ...
            {
                var newMaxSpeed = velocity.isExceeding(actualMaxSpeed) ? velocity.magnitude : actualMaxSpeed;
        ...
        /// </summary>
        private Vector3 ConstrainInputVector(Vector3 inputVector)
        {
            var worldUp = -GetGravityDirection();

            var inputVectorDotWorldUp = Vector3.Dot(inputVector, worldUp);
        ...
        /// </summary>
        private void CalcDesiredVelocity(float deltaTime)
        {
            // Current movement direction

            var movementDirection = Vector3.ClampMagnitude(GetMovementDirection(), 1.0f);

            // The desired velocity from animation (if using root motion) or from input movement vector

            var desiredVelocity = useRootMotion && rootMotionController
        ...
        /// </summary>
        private void ApplyDownwardsForce()
        {
            var groundRigidbody = characterMovement.groundRigidbody;
            if (!groundRigidbody || groundRigidbody.isKinematic)
                return;

            var downwardForce = mass * GetGravityVector();
        ...
        /// </summary>
        private void WalkingMovementMode(float deltaTime)
        {
        ...
        {
            var overlapped = characterMovement.CheckHeight(_unCrouchedHeight);
            return !overlapped;
        }
        ...
        /// </summary>
        private void FallingMovementMode(float deltaTime)
        {
            ...
            if (IsOnGround() && !IsOnWalkableGround())
            {
                ...
                var groundNormal = characterMovement.groundNormal;
                ...
                if (Vector3.Dot(desiredVelocity, groundNormal) < 0.0f)
                {
                    ...
                    var groundNormal2D = Vector3.ProjectOnPlane(groundNormal, worldUp).normalized;
            ...
            var verticalVelocity = Vector3.Project(characterMovement.velocity, worldUp);
            var lateralVelocity = characterMovement.velocity - verticalVelocity;
            ...
            var actualFallSpeed = maxFallSpeed;
            if (physicsVolume)
        ...
        {
            var gravityMagnitude = GetGravityMagnitude();
        ...
        /// </summary>
        private float GetMaxJumpHeightWithJumpTime()
        {
        ...
        private bool CanJump()
        {
            ...
            var isJumpAllowed = IsJumpAllowed();
            if (isJumpAllowed)
            {
                ...
                else
                {
                    ...
                    var jumpInputHeld = jumpInputPressed && jumpInputHoldTime < jumpMaxHoldTime;
        ...
        private bool DoJump()
        {
            ...
            var verticalSpeed = Mathf.Max(Vector3.Dot(characterMovement.velocity, worldUp), jumpImpulse);
        ...
        /// </summary>
        private void NotifyJumpApex()
        {
            if (!notifyJumpApex)
                return;

            var verticalSpeed = Vector3.Dot(GetVelocity(), -GetGravityDirection());
        ...
        /// </summary>
        private void FlyingMovementMode(float deltaTime)
        {
            if (useRootMotion && rootMotionController)
                characterMovement.velocity = GetDesiredVelocity();
            else
            {
                var actualFriction = IsInWaterPhysicsVolume() ? physicsVolume.friction : flyingFriction;
        ...
        /// </summary>
        private float CalcImmersionDepth()
        {
            var depth = 0.0f;

            if (IsInWaterPhysicsVolume())
            {
                var height = characterMovement.height;
                ...
                else
                {
                    ...
                    var rayOrigin = GetPosition() + worldUp * height;
                    var rayDirection = -worldUp;

                    var waterVolumeCollider = physicsVolume.boxCollider;
                    depth = !waterVolumeCollider.Raycast(new Ray(rayOrigin, rayDirection), out var hitInfo, height)
                        ? 1.0f
        ...
        /// </summary>
        private void SwimmingMovementMode(float deltaTime)
        {
            // Compute actual buoyancy factoring current immersion depth

            var depth = CalcImmersionDepth();
            var actualBuoyancy = buoyancy * depth;

            // Calculate new velocity

            var desiredVelocity = GetDesiredVelocity();
            var newVelocity = characterMovement.velocity;

            var worldUp = -GetGravityDirection();
            var verticalSpeed = Vector3.Dot(newVelocity, worldUp);
            ...
            else if (depth < 0.65f)
            {
                ...
                var verticalDesiredSpeed = Vector3.Dot(desiredVelocity, worldUp);
            ...
            if (useRootMotion && rootMotionController)
            {
                ...
                var verticalVelocity = Vector3.Project(newVelocity, worldUp);
            ...
            else
            {
                ...
                var actualFriction = IsInWaterPhysicsVolume()
        ...
        /// Called if MovementMode is set to Custom.
        /// </summary>
        private void CustomMovementMode(float deltaTime)
        {
        ...
        private void UpdateRotation(float deltaTime)
        {
            ...
            else if (_rotationMode == RotationMode.OrientRotationToMovement)
            {
                ...
                var shouldRemainVertical = IsWalking() || IsFalling();
        ...
        /// Called if RotationMode is set to Custom.
        /// </summary>
        private void CustomRotationMode(float deltaTime)
        {
