...
namespace GamePlay.Enemy.Spawn.Processor.Definition.Probability.Editor
{
    public class EnemyProbabilityAttributeDrawer : OdinAttributeDrawer<EnemyProbabilityAttribute>
    {
        ...
        private EnemyDefinitionMapperFactory _registry;
        ...
        protected override void Initialize()
        {
            ...
            var registries = new List<EnemyDefinitionMapperFactory>();
            var guids = AssetDatabase.FindAssets($"t:{typeof(EnemyDefinitionMapperFactory)}");

            foreach (var guid in guids)
            {
                var assetPath = AssetDatabase.GUIDToAssetPath(guid);
                var asset = AssetDatabase.LoadAssetAtPath<EnemyDefinitionMapperFactory>(assetPath);
        ...
        {
            var values =
                ValueResolver.Get<EnemyProbabilityDictionary>(Property, "_values").GetValue();
        ...
        private Rect[] DrawEntries(Rect rect, EnemyProbabilityDictionary values)
        {
        ...
        private void DrawSelection(Rect rect, Rect[] rects, EnemyProbabilityDictionary values)
        {
